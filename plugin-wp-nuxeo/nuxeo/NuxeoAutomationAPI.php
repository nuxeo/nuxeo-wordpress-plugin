...
class NuxeoPhpAutomationClient {
    ...
    public function getSession($username = 'Administrator', $password = 'Administrator') {
        $this->session = $username . ":" . $password;
				$this->login = $username;
        $session = new NuxeoSession($this->url, $this->session, $this->login);
        return $session;
    }
}
...
class NuxeoSession {
    ...
    public function NuxeoSession($url, $session, $username, $headers = "Content-Type: application/json+nxrequest") {
        ...
        $this->headers = $headers;
				$this->headers = array($this->headers, "remote_user: " . $username);
    }
